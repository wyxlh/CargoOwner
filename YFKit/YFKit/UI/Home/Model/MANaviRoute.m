...
+ (UIColor *)colorWithTrafficStatus:(NSString *)status
{
    ...
    if (colorMapping == nil)
    {
        colorMapping = @{@"未知":NavColor,
                         @"畅通":NavColor,
                         @"缓行":NavColor,
                         @"拥堵":NavColor};
    }
    
    return colorMapping[status] ?: NavColor;
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        UIColor *greenColor = [UIColor colorWithRed:40/255.0 green:196/255.0 blue:107/255.0 alpha:1];
        self.routeColor = [UIColor blueColor];
        self.walkingColor = [UIColor cyanColor];
        self.railwayColor = greenColor;
        
       
        
        self.trafficColors = @[greenColor, greenColor, [UIColor yellowColor], NavColor];
    }
