...
-(void)setModel:(YFDriverListModel *)model{
    ...
    self.name.text = [NSString stringWithFormat:@"%@    交易次数  %@次",[NSString getNullOrNoNull:model.driverName],model.transactionCount];
    if ([NSString isBlankString:model.carLawId]) {
        self.LicenseNum.text = @"车牌号码 : ";
    }else{
