...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 1) {
        ...
        cell.accessoryType                   = UITableViewCellAccessoryNone;
        cell.version.hidden                  = NO;
        cell.version.text                    = self.userName;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            //重置密码
            YFResetPassWordViewController *reset        = [[YFResetPassWordViewController alloc]initWithNibName:@"YFResetPassWordViewController" bundle:nil];
...
-(UIButton *)SignOutBtn{
    if (!_SignOutBtn) {
        ...
        _SignOutBtn.frame                        = CGRectMake(20, self.tableView.height-100, ScreenWidth-40, 50);
