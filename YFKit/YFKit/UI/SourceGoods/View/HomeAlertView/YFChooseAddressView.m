...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((ScreenWidth-47)/4,35);
}
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.selectIndex == 1) {
        ...
        self.selectCityCode                  = model.code;
        // 区的数据
        ...
        self.selectAreaCode                  = [model.address isEqualToString:@"全市"] ? @"" : model.code;
    }
    ...
    if (self.selectIndex < 3) {
        ...
        NSString *detailAddressStr,*detailAddressCode;
        if ([NSString isBlankString:self.selectAreaName]) {
            ...
            detailAddressCode                = [NSString stringWithFormat:@"%@/%@",self.selectProCode,self.selectCityCode];
            
        }else{
            detailAddressStr                 = [NSString stringWithFormat:@"%@/%@/%@",self.selectProName,self.selectCityName,self.selectAreaName];
            detailAddressCode                = [NSString stringWithFormat:@"%@/%@/%@",self.selectProCode,self.selectCityCode,self.selectAreaCode];
        }
        
        !self.chooseDetailAddressBlock ? : self.chooseDetailAddressBlock(detailAddressStr,detailAddressCode);
        [self disappear];
    }
