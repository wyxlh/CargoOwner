...
//    cell.addressView.hidden                     = !self.isChooseDriver;
//    cell.addressCons.constant                   = self.isChooseDriver ? 27.5f : 0.0f;
    if (self.dataArr.count != 0) {
    ...
    [[[cell.PlaceOrderBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(id x) {
        ...
        if (self.isChooseDriver) {
            ...
            place.driverListModel                = self.dataArr[indexPath.row];
...
-(YFNullView *)nullView{
    if (!_nullView) {
        ...
        [_nullView.noMoreBtn setTitle:@"暂无司机" forState:0];
        _nullView.frame                          = CGRectMake(0, 0, ScreenWidth, ScreenHeight-NavHeight-45);
