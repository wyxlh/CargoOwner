...
+ (instancetype)instance {
    if (!instance){
        ...
        [GiFHUD setGifWithImageName:@"Lodging.gif"];
    }
    return instance;
}
...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        self.imageView = [[UIImageView alloc] initWithFrame:CGRectInset(self.bounds, 15, 15)];
...
+ (void)showWithOverlay {
    ...
    [self dismiss:^{
        [APPDELEGATE.window addSubview:[[self instance] covering]];
        [self show];
    }];
}
...
+ (void)dismiss {
    [[[self instance] covering] removeFromSuperview];
